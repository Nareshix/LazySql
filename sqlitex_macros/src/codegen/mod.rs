...
use sqlitex_type_inference::{
    binding_patterns::get_type_of_binding_parameters, detect_query_cardinality,
    is_create_table, pg_cast_syntax_to_sqlite, rewrite_bool_columns,
...
pub(crate) fn expand(
    ...
    for field in fields.named.iter_mut() {
        ...
        if ident == "transaction" {
            return Err(syn::Error::new(ident.span(), "`transaction` is a reserved keyword. Rename this field to something else."));
        }

        if ident == "init" && db_path_lit.is_some() && db_path_lit.unwrap().value().ends_with(".sql") {
            return Err(syn::Error::new(ident.span(), "`init` is a reserved keyword when pointing to an external .sql file. Rename this field to something else."));
        }

        if ident == "migrate" && db_path_lit.is_some() && (std::path::Path::new(&db_path_lit.unwrap().value()).is_dir() || db_path_lit.unwrap().value().ends_with('/')) {
            return Err(syn::Error::new(ident.span(), "`migrate` is a reserved keyword when pointing to an external migrations folder. Rename this field to something else."));
        }

        let ident_str = ident.to_string();
        if ident_str.ends_with("_bulk") {
            let base_name = ident_str.strip_suffix("_bulk").unwrap();
            return Err(syn::Error::new(ident.span(), format!("`{}` has been reserved. This method is automatically generated for batch operations for `{}` method. Choose a different name.", ident_str, base_name)));
        }
        ...
        if let Some(sql_lit) = parse_sql_macro_type(&field.ty)? {
            ...
            let sql_query = rewrite_bool_columns(&sql_query).map_err(|msg| syn::Error::new(sql_lit.span(), msg))?;

            validate_no_virtual_tables(&sql_query).map_err(|msg| syn::Error::new(sql_lit.span(), msg))?;
            validate_cast_types(&sql_query).map_err(|msg| syn::Error::new(sql_lit.span(), msg))?;
            validate_create_table_types(&sql_query).map_err(|msg| syn::Error::new(sql_lit.span(), msg))?;
            ...
            if is_create_table(&sql_query) {
                ...
                generated_methods.push(create_table::generate_create_table(ident, field_attrs, &doc_comment));
                continue;
            }

            let select_types = get_types_from_select(&sql_query, &all_tables)
                .map_err(|err_msg| syn::Error::new(sql_lit.span(), format!("Return Type Error: {}", err_msg)))?;

            let binding_types = get_type_of_binding_parameters(&sql_query, &all_tables).map_err(|err| {
                ...
                if let Some(raw_line) = lines.get(line_idx) {
                    let indent_len_bytes = raw_line.char_indices().take_while(|(_, c)| c.is_whitespace()).last().map(|(i, c)| i + c.len_utf8()).unwrap_or(0);
                    let start_byte_idx = raw_line.chars().take(start_col).map(|c| c.len_utf8()).sum::<usize>();
                    let end_byte_idx = raw_line.chars().take(end_col).map(|c| c.len_utf8()).sum::<usize>();
                    let safe_indent = if indent_len_bytes <= start_byte_idx { indent_len_bytes } else { 0 };
                    ...
                    let padding: String = trimmed_line[..err_start_in_trimmed].chars().map(|c| if c == '\t' { '\t' } else { ' ' }).collect();
                    let arrows = "^".repeat(err_len.max(1));
                    msg = format!("{}\n\n{}\n{}{}", msg, trimmed_line, padding, arrows);
                }
                syn::Error::new(sql_lit.span(), msg)
            })?;
            ...
            for (i, param) in binding_types.iter().enumerate() {
                ...
                let is_keyword = matches!(
                    base_name.as_str(),
                    "type" | "match" | "let" | "fn" | "struct" | "enum" | "trait" | "impl" | "where" | "for" | "loop" | "while" | "if" | "else" | "return" | "break" | "continue" | "mut" | "ref" | "in" | "as" | "use" | "pub" | "const" | "static" | "move" | "async" | "await" | "dyn" | "self" | "super" | "crate"
                );
                if is_keyword {
            ...
            if !select_types.is_empty() {
                ...
                let (structs, methods) = read::generate_read_methods(
                    sql_lit.span(), struct_name, ident, field_attrs, &doc_comment, &binding_types, &param_names, &select_types, cardinality
                )?;
                generated_structs.push(structs);
                generated_methods.push(methods);
            } else {
                generated_methods.push(write::generate_write_methods(
                    sql_lit.span(), ident, field_attrs, &doc_comment, &binding_types, &param_names
                )?);
            }
            ...
            generated_methods.push(runtime::generate_runtime_method(
                ident, field_attrs, &doc_comment, &runtime_input
            ));
        } else {
            let ty = &field.ty;
