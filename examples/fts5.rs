...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    conn.execute_many_runtime("CREATE VIRTUAL TABLE articles USING fts5(title, body)")?;
    ...
    conn.execute_runtime("INSERT INTO articles VALUES ('Full text search', 'FTS5 allows powerful text searching in SQLite')")?;

    // Full text search
    let results =
        conn.query_runtime("SELECT title, body FROM articles WHERE articles MATCH 'SQLite'")?;
    ...
    let results = conn.query_runtime(
    ...
    // Phrase search
    let results =
        conn.query_runtime("SELECT title FROM articles WHERE articles MATCH '\"text search\"'")?;
    ...
    // Prefix search
    let results = conn.query_runtime("SELECT title FROM articles WHERE articles MATCH 'Rust*'")?;
